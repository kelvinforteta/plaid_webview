...
class PlaidWebview {
  ...
  PlaidWebview(Configuration configuration) {
...
class _WebViewPage {
  ...
  _processParams(Map<String, String> queryParams, String url) async {
    if (queryParams != null) {
      ...
      dynamic accountName = queryParams['account_name'];
      if (token != null && accountId != null) {
        if (!_stripeToken) {
          this._success(Result(token, accountId, queryParams, accountName));
        } else {
  ...
  _fetchStripeToken(String token, String accountId) async {
    ...
    Response responseAccessToken =
        await post(Uri.parse(_config.environmentPlaidPathAccessToken),
            body: json.encode({
              'public_token': token,
    ...
    Response responseStripeToken =
        await post(Uri.parse(_config.environmentPlaidPathStripeToken),
            body: json.encode({
              'client_id': this._config.plaidClientId,
              'secret': this._config.secret,
    ...
    _success(Result(stripeTokenData['stripe_bank_account_token'], null,
...
class Result {
  ...
  Result(this.token, this.accountId, this.response, this.accountName);
}
